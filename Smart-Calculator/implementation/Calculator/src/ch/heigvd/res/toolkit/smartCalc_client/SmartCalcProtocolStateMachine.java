...
public class SmartCalcProtocolStateMachine extends AbstractStateMachine
		implements IStateMachine {

	final static Logger LOG = Logger
			.getLogger(SmartCalcProtocolStateMachine.class.getName());
	...
	// @Override
	// public void run() {
	// Event maxIdleTimeReachedEvent = new
	// Event(PingPongProtocol.EventType.EVENT_TYPE_MAX_IDLE_TIME_REACHED);
	// ComputingEngProtocolStateMachine.this.onEvent(maxIdleTimeReachedEvent);
	// }
	// }, PingPongProtocol.MAX_INACTIVE_TIMEOUT,
	// PingPongProtocol.MAX_INACTIVE_TIMEOUT);
	// }
	//
	// public void stop() {
	...
	public IState getInitialState() {
		return SmartCalcProtocol.State.STATE_START_UP;
	}
	...
	public void onStateEntered(IState state) {
		...
		if (state == SmartCalcProtocol.State.STATE_START_UP) {
			// Normally here we have to send a multicast message: HELLO
			System.out.println("Sending HELLOOOO on multicast here");

			// Message msg = new
			// Message(PingPongProtocol.MessageType.MESSAGE_TYPE_NOTIFICATION);
			// msg.setAttribute("payload",
			// PingPongProtocol.NOTIFICATION_WELCOME_TEXT);
			// getContext().sendMessage(msg);
			triggerTransitionToState(SmartCalcProtocol.State.STATE_WAITING_RESP);
			// inactivityGuard.notifyClientActivity();
		}

		// /*
		// When we enter STATE_END, we send a notification to inform the client
		// that we are
		...
		// Message(PingPongProtocol.MessageType.MESSAGE_TYPE_NOTIFICATION);
		// msg.setAttribute("payload",
		// PingPongProtocol.NOTIFICATION_CLOSING_TEXT);
		// getContext().sendMessage(msg);
	...
	public void onEvent(Event e) {
		LOG.log(Level.INFO, "State Machine has received an event: {0} - {1}",
				new Object[] { e.getType(), e });

		// if (e.getType() ==
		// ComputingEngProtocol.EventType.EVENT_TYPE_MAX_IDLE_TIME_REACHED) {
		// Message timerNotification = new
		// Message(PingPongProtocol.MessageType.MESSAGE_TYPE_NOTIFICATION);
		// timerNotification.setAttribute("payload",
		// PingPongProtocol.NOTIFICATION_IDLE_TEXT);
		// getContext().sendMessage(timerNotification);
		// lateCount++;
		// return;
		// }

		// // If we have received an invalid message from the client we return
		// an error message
		// if (e.getType() ==
		// Event.EventType.EVENT_TYPE_INVALID_MESSAGE_ARRIVED) {
		// Message reply = new Message(ComputingEngProtocol.MessageType.);
		// reply.setAttribute("statusCode", 400);
		// reply.setAttribute("payload",
		// PingPongProtocol.RESULT_INVALID_CMD_TEXT);
		// getContext().sendMessage(reply);
		// return;
		// }

		// If we have received a message from the client, we have to update the
		...
		// Message(PingPongProtocol.MessageType.MESSAGE_TYPE_RESULT);
		// lastMessage.setAttribute("payload",
		// PingPongProtocol.RESULT_BYE_TEXT);
		...
		// // Whatever the state, if we receive a SCORE command, we know what to
		// do
		// if (e.getType() == Event.EventType.EVENT_TYPE_MESSAGE_ARRIVED) {
		// Message incomingMessage = (Message) (e.getAttribute("message"));
		// String command = (String) incomingMessage.getAttribute("command");
		...
		// // Whatever the state, if we receive a HELP command, we know what to
		// do
		// if (e.getType() == Event.EventType.EVENT_TYPE_MESSAGE_ARRIVED) {
		// Message incomingMessage = (Message) (e.getAttribute("message"));
		// String command = (String) incomingMessage.getAttribute("command");
		...
		// Message(PingPongProtocol.MessageType.MESSAGE_TYPE_RESULT);
		//
		// StringBuilder helpMessagePayload = new
		// StringBuilder("You can use the following commands: ");
		// for (PingPongProtocol.Command cmd :
		// PingPongProtocol.Command.values()) {
		...
		// switch ((SmartCalcProtocol.State) getCurrentState()) {
		// case STATE_START_UP:
	...
	// /**
	// * This method handles events when the state machine is either in
	// STATE_PING or STATE_PONG
	// * @param e the event
	// */
	...
	// Message reply = new
	// Message(ComputingEngProtocol.MessageType.MSG_HERE_I_AM);
	...
	// successful =
	// (command.equals(ComputingEngProtocol.Command.CMD_HELLO.getKeyword()));
	// targetState = ComputingEngProtocol.State.STATE_RESPONDING;
	// }
	// // else {
	// // successful =
	// (command.equals(PingPongProtocol.Command.CMD_PONG.getKeyword()));
